...
local InstaReviveToggle = nil
local InstantReturnToggle = nil
local AutoSafeReviveToggle = nil
local SpeedSliderElement = nil
...
local function CreateCustomAnnouncement()
    ...
    ContentLabel.Text = "HI ALL USERS, THE SCRIPT HAS JUST RETURNED!@SkylerModz\n\n©Ascynthub New HUB for Multi-Games"
...
local function LoadMainScript(keyInfo)
    ...
    SpeedSliderElement = PlayerTab:CreateSlider({
        Name = "Set Speed",
        Range = {1450, 12500},
        Increment = 50,
        Suffix = "",
    ...
    InstaReviveToggle = MainTab:CreateToggle({
        Name = "Auto Revive Others (Close Range)",
        CurrentValue = false,
        Flag = "InstaRevive",
        Callback = function(state)
    ...
    InstantReturnToggle = MainTab:CreateToggle({ 
        Name = "Auto Carry & Instant Return (Flash TP)",
        CurrentValue = false,
        Flag = "InstantReturnMode",
        Callback = function(state)
            instantReturnEnabled = state
        end
    })
    
    AutoSafeReviveToggle = MainTab:CreateToggle({
...
task.spawn(function()
    while true do
        ...
        if hrp then
            ...
            if instaReviveEnabled and not isProcessingRevive then
                local folder = Workspace:FindFirstChild("Game") and Workspace.Game:FindFirstChild("Players")
                if folder then
            ...
            if instantReturnEnabled and not isProcessingRevive then
                local folder = Workspace:FindFirstChild("Game") and Workspace.Game:FindFirstChild("Players")
                if folder then
                    for _, otherChar in ipairs(folder:GetChildren()) do
                        if otherChar:IsA("Model") and otherChar.Name ~= LocalPlayer.Name and otherChar:GetAttribute("Downed") == true then
                            ...
                            if otherHrp then
                                ...
                                hrp.CFrame = otherHrp.CFrame + Vector3.new(0, 1.5, 0)
                                task.wait(0.04) 
...
UserInputService.InputBegan:Connect(function(input, gameProcessedEvent)
    ...
    if input.KeyCode == Enum.KeyCode.K then
        if InstaReviveToggle then
            local targetState = not instaReviveEnabled
            InstaReviveToggle:Set(targetState)
            Rayfield:Notify({
                Title = "Keybind Presionada [K]",
                Content = "Instant Revive " .. (targetState and "ACTIVADO" or "DESACTIVADO"),
                Duration = 2,
                Image = 4483362458,
            })
        end
    ...
    elseif input.KeyCode == Enum.KeyCode.J then
        if InstantReturnToggle then
            local targetState = not instantReturnEnabled
            InstantReturnToggle:Set(targetState)
            Rayfield:Notify({
                Title = "Keybind Presionada [J]",
                Content = "Flash TP Return " .. (targetState and "ACTIVADO" or "DESACTIVADO"),
                Duration = 2,
                Image = 4483362458,
            })
        end
    ...
    elseif input.KeyCode == Enum.KeyCode.G then
        if AutoSafeReviveToggle then
            local targetState = not autoReviveEnabled
            AutoSafeReviveToggle:Set(targetState)
            Rayfield:Notify({
                Title = "Keybind Presionada [G]",
                Content = "Auto TP & Safe Carry " .. (targetState and "ACTIVADO" or "DESACTIVADO"),
                Duration = 2,
                Image = 4483362458,
            })
        end
    ...
    elseif input.KeyCode == Enum.KeyCode.H then
        if SpeedSliderElement then
            SpeedSliderElement:Set(2430)
            Rayfield:Notify({
                Title = "Keybind Presionada [H]",
                Content = "Velocidad cambiada a 2430",
                Duration = 2,
                Image = 4483362458,
            })
        end
